    ...
    # if False:
        ...
        for batch in tqdm.tqdm(loader):
            ...
            with torch.no_grad(),torch.cuda.amp.autocast():
                batch_features, batch_tokens = clip_model.encode_image(images)
                batch_features = torch.cat([batch_features.unsqueeze(1), batch_tokens], dim=1)
        ...
        index_features = torch.stack(index_features,dim=0)
...
def generate_predictions(
    device: torch.device, args: argparse.Namespace, clip_model: clip.model.CLIP, qwen_model: callable, qwen_preprocess: callable, query_dataset: torch.utils.data.Dataset, preload_dict: Dict[str, Union[str,None]], **kwargs
    ...
    if preload_dict['captions'] is None or not os.path.exists(preload_dict['captions']):
        all_captions, relative_captions, modified_captions = [], [], []
        ...
        for batch in query_iterator:
            ...
            caption = [f'a photo of a dress']
            modified_caption = relative_captions[-1]
            image_path1 = f'Reference image 1 path'
            image_path2 = f'Reference image 2 path'  

            image_path3 = batch['reference_img_path'][0][0]  
            messages = [
                {
                    "role": "user",
                    "content": [
                        ...
                        # {"type": "text", "text": f"""You are an image description assistant. Please analyze the given image carefully and provide a detailed description of its contents.\n\nAnswer: """},
                        {"type": "text", "text": f"""Editing instruction: "is green with a four leaf cloverv".\nEdited image: The image shows a green t-shirt with a white four-leaf clover design on the front. The clover is centered on the shirt and appears to be slightly distressed or worn, giving it a vintage look. The t-shirt has short sleeves and a crew neck. The background of the image is plain, focusing attention on the t-shirt and its design."""},
                        {
                            "type": "image",
                            "image": image_path2,
                        },
                        # {"type": "text", "text": f"""You are an image description assistant. Please analyze the shirt in the given image carefully and provide a detailed description of its contents.\n\nAnswer: """},
                        {"type": "text", "text": f"""Editing instruction: "is a long sleeve tan brown shrit with buttons".\nEdited image: The image shows a person wearing a long-sleeved, brown shirt. The shirt features a ribbed neckline, cuffs, and hem, giving it a casual and comfortable look. There are three buttons on the placket, which is a vertical row of buttons that runs down the front of the shirt. The shirt also has a single chest pocket on the left side, which is a common design element in henley shirts."""},
                        {
                            "type": "image",
                            "image": image_path3,
                        },
                        # {"type": "text", "text": f"""You are an image description assistant. Please analyze the shirt in the given image carefully and provide a detailed description of its contents.\n\nAnswer: """},
                        {"type": "text", "text": f"""Editing instruction: "{relative_captions[-1]}".\nEdited image:"""},

                        ],
                            }
                        ]
            
            messages = [messages for _ in range(1)]
            text = [qwen_preprocess.apply_chat_template(
                msg, tokenize=False, add_generation_prompt=True) for msg in messages]
            image_inputs, video_inputs = process_vision_info(messages)
            inputs = qwen_preprocess(
                text=text,
                images=image_inputs,
                videos=video_inputs,
                padding=True,
            ...
            output_text_list = []
            for _ in range(5):
                generated_ids = qwen_model.generate(**inputs, max_new_tokens=70, do_sample=True, temperature=0.8, top_k=50, top_p=0.8)
                # generated_ids = model.generate(**inputs, max_new_tokens=512)
                generated_ids_trimmed = [
                    out_ids[len(in_ids) :] for in_ids, out_ids in zip(inputs.input_ids, generated_ids)
                ]
                output_text = qwen_preprocess.batch_decode(
                    generated_ids_trimmed, skip_special_tokens=True, clean_up_tokenization_spaces=False
                )
                output_text_list.extend(output_text)

            each_modified = []
            for each_text in output_text_list:
                if len(each_text) > 10:
                    each_modified.append(each_text)
            if len(each_modified) == 0:
                each_modified.append(relative_captions[-1])
            # print(f'Original Caption: {relative_captions[-1]}, Modified Caption: {modified_caption}')
            all_captions.extend(caption)
            modified_captions.append(each_modified)
        ...
        all_captions, modified_captions, gt_img_ids, relative_captions, target_names, reference_names, query_ids = res_dict.values()


    modified_captions1 = [each[0] for each in modified_captions]
    predicted_features1 = text_encoding(device, clip_model, modified_captions1, batch_size=batch_size, mode=args.retrieval)

    modified_captions2 = [each[1] for each in modified_captions]
    predicted_features2 = text_encoding(device, clip_model, modified_captions2, batch_size=batch_size, mode=args.retrieval)

    modified_captions3 = [each[2] for each in modified_captions]
    predicted_features3 = text_encoding(device, clip_model, modified_captions3, batch_size=batch_size, mode=args.retrieval)

    modified_captions4 = [each[3] for each in modified_captions]
    predicted_features4 = text_encoding(device, clip_model, modified_captions4, batch_size=batch_size, mode=args.retrieval)

    modified_captions5 = [each[4] for each in modified_captions]
    predicted_features5 = text_encoding(device, clip_model, modified_captions5, batch_size=batch_size, mode=args.retrieval)

    predicted_features6 = text_encoding(device, clip_model, relative_captions, batch_size=batch_size, mode=args.retrieval)

    predicted_features = torch.stack([predicted_features1, predicted_features2, predicted_features3, predicted_features4, predicted_features5, predicted_features6], dim=0)
    print(f'Predicted Features Shape: {predicted_features.size()}')
    return {
