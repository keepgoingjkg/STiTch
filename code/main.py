...
class Augmenter(object):
    def __init__(self, preprocess, n_views=2):
...
def main():
    ...
    #### for qwen model
    parser.add_argument("--qwen_model_path", default="Qwen_path", type=str,
    help='pre-trained weights path of qwen model')
    parser.add_argument("--qwen_min_pixels", default=128*28*28, type=int,
    help='min_pixels for qwen model')
    parser.add_argument("--qwen_max_pixels", default=256*28*28, type=int,
    help='max_pixels for qwen model')
    ...
    if len(args.preload):
        os.makedirs('precomputed_multi', exist_ok=True)
    if 'img_features' in args.preload:
        ...
        preload_dict['img_features'] = os.path.join('precomputed_multi', preload_str + 'openai_ot_img_features.pkl')
    ...
    if 'captions' in args.preload:
        ...
        if args.blip_prompt != 'prompts.blip_prompt':
            preload_dict['captions'] = os.path.join('precomputed_multi', caption_load_str + f'_captions_{args.blip_prompt.split(".")[-1]}.pkl')
        else:
            preload_dict['captions'] = os.path.join('precomputed_multi', caption_load_str + '_captions.pkl')
    ...
    if 'mods' in args.preload:
        ...
        preload_dict['mods'] = os.path.join('precomputed_multi', mod_load_str + f'_mods_{args.llm_prompt.split(".")[-1]}.json')
    ...
    if args.clip in ['ViT-bigG-14','ViT-B-32','ViT-B-16','ViT-L-14','ViT-H-14','ViT-g-14']:
        ...
        clip_model, _, clip_preprocess = open_clip.create_model_and_transforms(args.clip, pretrained=pretraining[args.clip])
    ...
    qwen_model = Qwen2VLForConditionalGeneration.from_pretrained(
        args.qwen_model_path, torch_dtype="auto", device_map=device,
    )
    aug_preprocess = Augmenter(preprocess, n_views=10)
    qwen_processor = AutoProcessor.from_pretrained(args.qwen_model_path, min_pixels=args.qwen_min_pixels, max_pixels=args.qwen_max_pixels)
    target_datasets, query_datasets, pairings = [], [], []

    if 'fashioniq' in args.dataset.lower():
        dress_type = args.dataset.split('_')[-1]
        target_datasets.append(datasets.FashionIQDataset(args.dataset_path, args.split, [dress_type], 'classic', aug_preprocess, blip_transform=preprocess))
        query_datasets.append(datasets.FashionIQDataset(args.dataset_path, args.split, [dress_type], 'relative', aug_preprocess, blip_transform=preprocess))
        ...
        target_datasets.append(datasets.CIRRDataset(args.dataset_path, split, 'classic', aug_preprocess, blip_transform=preprocess))
        query_datasets.append(datasets.CIRRDataset(args.dataset_path, split, 'relative', aug_preprocess, blip_transform=preprocess))
        compute_results_function = compute_results.cirr
        pairings.append('default')

    elif args.dataset.lower() == 'circo':
        target_datasets.append(datasets.CIRCODataset(args.dataset_path, args.split, 'classic', preprocess, blip_transform=preprocess))
        query_datasets.append(datasets.CIRCODataset(args.dataset_path, args.split, 'relative', preprocess, blip_transform=preprocess))
    ...
    for query_dataset, target_dataset, pairing in zip(query_datasets, target_datasets, pairings):
        ...
        input_kwargs = {
            ...
            'qwen_model': qwen_model, 'preprocess': preprocess, 'device': device, 'split': args.split,
            'qwen_preprocess': qwen_processor, 'preload_dict': preload_dict,
        }
        ...
        index_features, index_names, index_ranks, aux_data = utils.extract_image_features(
            device, args, target_dataset, clip_model, batch_size=1, preload=preload_dict['img_features'])
        ...
        print(f'Generating conditional query predictions (CLIP: {args.clip}, Qwen: Qwen-VL-7B).')
